	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for (int i = 0; i< trials; i++){
				BasicDocument basicdoc = new BasicDocument(readfile);
				Double fscore  = basicdoc.getFleschScore();
			}
			long endtime = System.nanoTime();
			double esttime  =  (endtime - start) / 1000000000.0 / trials;
			System.out.println(esttime + '\t');

			long starttimeE  = System.nanoTime();
			for (int i = 0; i< trials; i++){
				EfficientDocument effdoc = new EfficientDocument(readfile);
				Double fscore  = effdoc.getFleschScore();
			}
			long endtimeE = System.nanoTime();
			double esttimeE  =  (endtimeE - starttimeE) / 1000000000.0 / trials;
			System.out.println(esttimeE + '\n');
